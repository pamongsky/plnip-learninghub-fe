...
export const classChatApi = {
  ...
  getMessages: async (classId: number, params?: { page?: number; per_page?: number }) => {
  ...
  sendMessage: async (classId: number, data: SendMessageData) => {
  ...
  getQuestions: async (classId: number, params?: { status?: 'all' | 'answered' | 'unanswered' }) => {
    const response = await api.get<{ data: ClassMessage[] }>(
  ...
  getQuestionStats: async () => {
    const response = await api.get<{ data: QuestionStats }>('/instructor/question-stats');
    return response.data.data;
  },
};
